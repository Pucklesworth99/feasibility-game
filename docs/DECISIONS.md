...
Feasibility = commissioning studies (metallurgy, geotech, water, permitting, mining) quarter by quarter, choosing a consultant tier per study: cheap ("a bloke from the pub with a spreadsheet") = fast/cheap but error-prone, triggers rework events and the market discounts the numbers; mid-tier = fine; **"The Good Consultants" = Tom's actual firm as a branded cameo** — dearer, accurate, catches fatal flaws early, market believes their numbers. The end screen turns the cameo into the business card: "The Good Consultants are real — [link]." The game's lead-gen CTA is diegetic. **Built 2026-07-04** — placeholders for the real firm name/URL live in `src/core/branding.ts`.

## D-019 · Build mode + juice: the sterilization lesson, playable
**Decided & built:** 2026-07-04
After publishing the DFS, the player sites the Process Plant, Tailings Dam and Camp on the map. A dashed pit outline shows the pit they can PREDICT (classified tiles + one pushback ring); construction reconciles against the pit the OREBODY wants (true ore + ring). Anything built inside the true pit → $6M relocation and a war-story debrief line; TSF adjacent to the creek → $8M regulator penalty. After first gold, the map reveals the true pit over their buildings. Juice pass: synthesized WebAudio chimes (zero assets, mutable via 🔊), screen shake on bonanza hits and market re-rates, stat pulses on change.

## D-020 · The McDonald's-game look: big footprints, fat outlines, a living map
**Decided:** 2026-07-04 (Tom: "still a bit dull… camp and mine need to be bigger" — reference: Molleindustria's McDonald's Videogame land quadrant)
Buildings occupy real multi-tile footprints (Plant 2×2, Tailings 2×2, Camp 2×1) on cleared earthworks pads, drawn big and chunky with fat dark outlines — placement validates the whole footprint (terrain, overlap, planned pit), and sterilization checks every footprint tile. Terrain palette saturated hard; findings overlay louder. Ambient animation loop (~11 fps interval-driven — rAF stalls in hidden/throttled contexts): plant smoke, pond shimmer, working rigs with dust on planned tiles, gold glints on Measured ground. Once first gold pours, the pit renders as excavated benches (rim + darker floor) instead of a dashed outline, swallowing the drill collars inside it.

## D-021 · FUN-FIRST REBUILD (v3): arcade runs, one verb, full satire — see FUN-AUDIT.md
**Decided:** 2026-07-04 (Tom: "quality isn't there… this isn't super fun, which should be the primary aim")
Full audit found v2 to be an authentic but joyless compliance simulator: dopamine batched into modals, no escalation, no near-misses, satire trapped in tooltips. v3 rebuilds the interaction layer around **tap-to-drill as a slot machine** (the Core Tray reveal), one verb + three buttons, a milestone track instead of quarters, arcade runs (3–6 min, score = market cap) with a persistent upgrade shop, and full Molleindustria-style visual satire (Barry, the Broker, the Banker, the Regulator, kangaroos). Supersedes D-001 (career → arcade+meta) and the quarterly-loop half of D-017; the sim core, JORC logic, market irony, build phase and business card all survive underneath. **Full spec: [FUN-AUDIT.md](FUN-AUDIT.md). Acceptance test #1: the naked drill-tap must be fun before anything else gets built.**

## D-022 · v4 "A Tiny Mine That Runs": the ambient operating diorama — see GAME-DESIGN-v4.md
**Decided:** 2026-07-04 (Tom: "the gameplay loop needs to be simple like Pixel Hotel… deep reasoning… architect the full thing before we build. Fun, simple, funny")
The Pixel Hotel reference names what v3 still missed: spike dopamine (the Core Tray) needs an **ambient cozy layer** — the mine as a living diorama that runs by itself. Two loops: DISCOVER (tap-to-drill, from D-021) + OPERATE (excavator chews the pit, haul trucks do loops, plant chuffs, gold pours pop for tap-collection, workers/roos animate the site). They feed each other like real mining and one run = one tenement's life (~5–8 min) ending in montage → score → business card → shop. Money simplified to one cash counter (Broker arrives in a ute for raises; megaphone keeps the announcement joke; market sim runs underneath as score engine only). Staff are hired characters, not stats — Barry available in every role at half price with 30% comedic failure. Market gap identified: Idle Miner Tycoon's demand × Farming Simulator's authenticity × Kairosoft's vacant mining slot × Molleindustria's satire, seeded through mining-industry LinkedIn. New engineering: dumb waypoint actor system; everything else survives from v2/v3. Build gates locked in the doc — **each gate passes on Tom's thumb before the next starts.**
